 ...
 */
public class ExamHard implements Initializable {
   ...
   // public RadioButton[] box = new RadioButton[120];
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        x.close();
        noOfQuestions = cnt;
        ExtraClass.controller.noOfHardQuestions = cnt;
    ...
    public void ShowAnswersButtonClicked(){
        try{
            x = new Scanner(new File("CorrectAnswersHard.txt"));
        }catch(Exception e){
            System.out.println("Error");
        }
        int ii=0;
        ...
        int flag=0;
        int kkk=0;
        for(int i=0;i<((controller.noOfHardQuestions)*4);i++){
            if(flag==0) {
                String s = answers[j];
    ...
    public void SubmitButtonClicked() throws Exception{
        ...
        if(box2.get(83).isSelected()) Score++;
        if(box2.get(86).isSelected()) Score++;
        if(box2.get(91).isSelected()) Score++;
        if(box2.get(93).isSelected()) Score++;
        if(box2.get(98).isSelected()) Score++;
        if(box2.get(100).isSelected()) Score++;
        if(box2.get(105).isSelected()) Score++;
        if(box2.get(110).isSelected()) Score++;
        if(box2.get(115).isSelected()) Score++;
        if(box2.get(119).isSelected()) Score++;*/
        ...
        String info = Controller.usernamefinal +":" + "Hard"+":"+ Score;
